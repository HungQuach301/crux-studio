...
2. Thu thập: PR merged trong 24 giờ qua theo làn; PR đang mở và trạng thái CI; PR đang **kẹt ở hàng đợi
   ...
   `ops/scripts/pr-triage.ts`, đọc `ops/logs/platform/P-016.jsonl` bằng `readRunLogs`; PR có nhãn automerge-delayed kèm SỐ GIỜ CÒN LẠI trước khi tự merge; các mục parked;
...
3. Mở issue "🤖 [Bản tin] YYYY-MM-DD", nhãn digest, tiếng Việt, tối đa khoảng 25 dòng, theo đúng bốn phần:
   ...
   Đang chờ merge
     ...
     Từ lượt kẹt liên tiếp thứ 3 trở đi (đọc
     `ops/logs/platform/P-016.jsonl` bằng `readRunLogs`, đừng tự `cat`; chuỗi lấy bằng `stuckStreak`),
     thêm "· N lượt liên tiếp không tự giải được" ngay trên dòng đó, để nó không im lặng như đã từng
     xảy ra (nhóm Z). Đếm CẢ HAI cách kẹt: `aborted-ineligible` và `red-after-merge` (gộp sạch rồi đỏ,
     mục P-025) — PR #81 kẹt theo cách thứ hai ba lượt liên tiếp mà không dòng nào nói ra. PR mang nhãn
