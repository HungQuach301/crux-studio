...
| G21 | Đường xác thực `videos.insert` là OAuth client scope `youtube.upload` cộng refresh token, và `videos.insert` có bucket quota riêng — trần **100 video/ngày** | `tài liệu nói vậy` (vế "In production sống lâu bao nhiêu" vẫn `suy luận`) | chủ dự án đã chốt **A** trên `#248` kèm ba điều kiện · tài liệu và Console khớp nhau ở **100**; ước tính ~6/ngày của `#248` là **sai** · chưa đo trên project thật | `VF-G21` |
...
## G21 · Đường xác thực YouTube cho `videos.insert`, và hạn mức tải lên

- **Nội dung:** hai vế, và chúng chịu tải khác nhau.
  1. **Đường xác thực.** Để `videos.insert` chạy được từ Actions cần một **OAuth 2.0 client** của một project Google Cloud có bật YouTube Data API v3, scope `https://www.googleapis.com/auth/youtube.upload`, cộng một **refresh token** cất trong GitHub Secrets. API key trần **không** dùng được cho `videos.insert` — nó ghi lên tài sản của một người dùng, nên bắt buộc OAuth. Refresh token chỉ **dài hạn** khi màn hình đồng ý của app ở chế độ **In production**; ở chế độ **Testing** nó hết hạn sau **7 ngày**.
  2. **Hạn mức tải lên là một bucket riêng, và trần của nó là 100 video mỗi ngày.** Cấp mặc định của một project bật YouTube Data API gồm **100 lần `search.list`**, **100 lần `videos.insert`**, và **10.000 đơn vị mỗi ngày cho *các endpoint còn lại*** — `videos.insert` **không** tiêu vào bể 10.000 đơn vị đó. Một lần `videos.insert` tốn **1 đơn vị trong bucket `Video Uploads`**. Trần tải lên vì thế là **100 video/ngày**, đúng bằng con số `Video Uploads per day 100` mà chủ dự án đọc từ Console ngày `2026-09-24`.

     ⚠️ **Con số "~6 video/ngày" trong phương án A của `#248` là SAI, và sổ này ghi lại để nó không quay lại.** Nó suy từ `10.000 đơn vị ÷ ~1.600 đơn vị mỗi lần tải`, tức áp mô hình quota của *các endpoint khác* cho `videos.insert` — một phép chia không có cơ sở, vì `videos.insert` có bucket riêng. Chủ dự án nói đúng khi bảo con số Console **khác** ước tính đó: hai con số **thật sự mâu thuẫn**, và con số đúng là **100**. Một lượt agent trước đã khai ngược lại điều này (comment `5829566818` trên `#248`); chỗ sai đã được đính chính bằng comment sau đó.
- **Vì sao nó chịu tải:** tiêu chí xong thứ nhất của `release/R-002` đòi quota mỗi lần tải được **đo**, không ước lượng — nên mọi con số ở đây đi thẳng vào một tiêu chí xong. Vế 1 nặng hơn vế 2: sai chế độ app (Testing thay vì In production) thì đường tải lên **tự chết sau 7 ngày** mà không chỉ báo nào đỏ ở lượt thứ nhất — đúng nhóm **Z** của `ops/known-failures.md`, vì lần chạy đầu vẫn xanh. Vế 2 sai thì hậu quả là một lịch phát hành lập trên con số sai, phát hiện được ngay lần chạm hạn mức đầu tiên.
- **Nguồn — tài liệu nhà cung cấp, đọc thật `2026-09-25`, kèm mốc đo:**

  | Nguồn | Trích nguyên văn |
  |---|---|
  | `developers.google.com/youtube/v3/getting-started` (HTTP 200) | *"Projects that enable the YouTube Data API have a default quota allocation of 100 search.list calls, 100 videos.insert calls, and 10,000 units per day combined for all other endpoints."* |
  | `developers.google.com/youtube/v3/docs/videos/insert` (HTTP 200) | *"Quota impact: 100 calls per day. A call to this method has a quota cost of 1 unit in the Video Uploads quota bucket."* |
  | `developers.google.com/identity/protocols/oauth2` (HTTP 200) | *"A Google Cloud Platform project with an OAuth consent screen configured for an external user type and a publishing status of 'Testing' is issued a refresh token expiring in 7 days, unless the only OAuth scopes requested are a subset of name, email address, and user profile"* — `youtube.upload` **không** thuộc tập con đó, nên mốc 7 ngày áp cho ca này. |

  Con số `Video Uploads per day 100` cũng được **chủ dự án đọc từ Cloud Console** ngày `2026-09-24` (`#248`) — hai nguồn độc lập cho cùng một số.

  **Một câu sai đã bị xoá khỏi mục này, ghi lại để không ai viết lại nó:** bản đầu của `G21` khai *"trang tài liệu nhà cung cấp không đọc được từ phiên cloud — cùng chính sách mạng đã chặn 16/18 đích ở `G7`"*. Câu đó **chưa từng được đo** và **sai**: phép đo của `G7` (`2026-09-21`) chạy trên một danh sách 18 đích **không có** `developers.google.com`, và nó còn tự mâu thuẫn với chính `G7` vì `cloud.google.com` là một trong **hai đích MỞ** mà `G7` ghi. Đo lại `2026-09-25`: cả ba URL trên trả **200**. Đây đúng là lối suy luận mà luật 3 ở đầu file loại bỏ — đọc tài liệu mất 30 giây, và việc thay nó bằng một suy luận là nguyên nhân gốc của con số sai ở trên.
- **Độ tin cậy:** `tài liệu nói vậy` — và nhãn này chỉ đúng **sau khi** ba trang ở trên được đọc thật. Tách theo vế, để không vế nào núp dưới nhãn chung:
  - hạn mức (vế 2) và mốc 7 ngày của chế độ Testing: `tài liệu nói vậy`, nguồn ở bảng trên;
  - *"In production thì refresh token sống lâu **bao nhiêu**"*: tài liệu chỉ nói Testing hết hạn sau 7 ngày, **không** hứa một thời hạn cho In production. Vế này vẫn là `suy luận`, và nó là vế duy nhất của `G21` chưa có nguồn — xem gạch đầu dòng *Dự phòng* chỗ "KHÔNG phủ".
- **Phần phụ thuộc:** `ops/lanes/release/backlog.md` (R-002) · `ops/lanes/verify/backlog.md` (VF-G21)
- **Cách kiểm:** một lần `videos.insert` thật, rồi đọc lại trang Quotas của Console trước và sau để lấy **hiệu số đơn vị của đúng lời gọi đó**. Chỉ chạy được sau khi chủ dự án làm xong điều kiện (1) của `#248` — tạo kênh, tạo OAuth client, cấp refresh token. **Cố ý không** đăng ký vào `pnpm recheck:assumptions`: bài kiểm này cần gọi ra ngoài và tiêu quota thật, mà các lệnh kiểm của repo phải chạy được khi không có mạng.
- **Dự phòng — lớp một đã viết sẵn, lớp hai CHƯA có code (khai rõ, không để câu khai đứng một mình):**
  - **Lớp một (cả hai vế):** đúng khuôn `G19` và `G20` — **không con số nào của mục này được nằm trong code.** Trần `100 video/ngày`, chi phí `1 đơn vị` mỗi lần tải, và bể `10.000 đơn vị` của các endpoint khác đều là **dữ liệu** kèm trường `source`, để giả định sai thì sửa một số trong một file dữ liệu chứ không sửa dòng code nào. Điều này còn là **ràng buộc lên lượt xây `R-002`**, không phải một mô tả về code đã có: xưởng `release` đang ở `impl: stub` và **chưa có file dữ liệu nào cho quota tải lên**. Lượt nào viết đường tải lên thật mà nhúng thẳng một con số vào code là lượt phá lớp dự phòng này.
  - **Lớp hai (đọc hạn mức thật ra từ phản hồi `403 quotaExceeded` của nhà cung cấp):** **CHƯA tồn tại.** Đo `2026-09-25`: `grep -rn "quotaExceeded" workshops/ kernel/ ops/scripts/` ra **0 kết quả**. Nó chỉ viết được khi có đường gọi API thật, tức sau điều kiện (1) của `#248`. Tới lúc đó, dự phòng của G21 **chỉ có lớp một** — và theo `CLAUDE.md` mục 7 thì thế là đủ cho phần đang chạy, vì phần đang chạy **không gọi API**: tiêu chí xong thứ hai của `R-002` (cổng `pnpm check:visibility`) là kiểm tĩnh trên code, không đụng mạng.
  - **Chỗ dự phòng KHÔNG phủ, khai thẳng:** tài liệu nói Testing hết hạn sau 7 ngày nhưng **không hứa** một thời hạn nào cho In production. Nếu In production cũng không giữ refresh token đủ lâu thì không lớp nào trong repo đỡ được — đường tải lên tự động sẽ cần một người bấm lại định kỳ, và đó là thứ mặc định **M8** của CHARTER tồn tại để loại bỏ. Ca đó là một quyết định mới, không phải một dự phòng.
- **Trạng thái:** `tài liệu nói vậy`, giao làn `verify`, mục `VF-G21`. **Chưa có lời gọi nào** — kênh YouTube chưa tạo, OAuth client chưa tạo, secret chưa có. Vế 2 nay có **hai nguồn khớp nhau** (tài liệu và Console), nhưng chưa nguồn nào là **phép đo của chính project này**: điều kiện (3) của `#248` vẫn còn nguyên giá trị, nó xác nhận project này đúng là đang ở cấp mặc định. Vế 1 chưa có lượt nào chạy qua mốc 7 ngày nên phần "In production giữ token dài hạn" **chưa từng được quan sát**.
