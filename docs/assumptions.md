...
| G21 | Đường xác thực `videos.insert` là OAuth client scope `youtube.upload` cộng refresh token dài hạn, và hạn mức tải lên có bucket riêng | `tài liệu nói vậy` | chủ dự án đã chốt **A** trên `#248` kèm ba điều kiện · refresh token chỉ dài hạn khi app ở **In production** · hai con số hạn mức CHƯA đo | `VF-G21` |
...
- **Nội dung:** hai vế, và chúng chịu tải khác nhau.
  ...
  2. **Hạn mức tải lên có bucket riêng.** Ngoài hạn mức đơn vị mỗi ngày của cả project, YouTube còn một hạn mức riêng đếm **số video mỗi ngày** — Console ngày `2026-09-24` ghi `Video Uploads per day 100` (chủ dự án đọc, `#248`). Hai con số này **không mâu thuẫn nhau**: chúng là hai bucket khác nhau, và bucket nào chạm trước thì bucket đó là cửa dừng thật.
...
- **Nguồn:** issue `🤖 [QĐ]` **#248**, phương án **A** cộng comment trả lời của chủ dự án `2026-09-24T23:50:04Z`. Con số `Video Uploads per day 100` là **chủ dự án đọc từ Cloud Console** ngày `2026-09-24`, không phải agent đọc. Trang tài liệu nhà cung cấp **không đọc được từ phiên cloud** — cùng chính sách mạng đã chặn 16/18 đích ở `G7`.
- **Độ tin cậy:** `tài liệu nói vậy`
...
- **Dự phòng — lớp một đã viết sẵn, lớp hai CHƯA có code (khai rõ, không để câu khai đứng một mình):**
  - **Lớp một (cả hai vế):** đúng khuôn `G19` và `G20` — **không con số nào của mục này được nằm trong code.** Cả hai hạn mức và chi phí đơn vị mỗi lần tải là **dữ liệu** kèm trường `source`, để giả định sai thì sửa một số trong một file dữ liệu chứ không sửa dòng code nào. Điều này còn là **ràng buộc lên lượt xây `R-002`**, không phải một mô tả về code đã có: xưởng `release` đang ở `impl: stub` và **chưa có file dữ liệu nào cho quota tải lên**. Lượt nào viết đường tải lên thật mà nhúng thẳng một con số vào code là lượt phá lớp dự phòng này.
  ...
  - **Chỗ dự phòng KHÔNG phủ, khai thẳng:** nếu vế 1 sai theo hướng "In production cũng không giữ refresh token dài hạn" thì không lớp nào trong repo đỡ được — đường tải lên tự động sẽ cần một người bấm lại định kỳ, và đó là thứ mặc định **M8** của CHARTER tồn tại để loại bỏ. Ca đó là một quyết định mới, không phải một dự phòng.
- **Trạng thái:** `tài liệu nói vậy`, giao làn `verify`, mục `VF-G21`. **Chưa có lời gọi nào** — kênh YouTube chưa tạo, OAuth client chưa tạo, secret chưa có. Vế 2 hiện là **một con số của Console đọc một lần**, chưa đối chiếu với hiệu số đơn vị của một lần tải thật; vế 1 chưa có lượt nào chạy qua mốc 7 ngày nên phần "In production giữ token dài hạn" chưa từng được quan sát.
