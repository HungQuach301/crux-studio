...
- ⬜ **Vì sao `parked`:** cách kiểm ở trên là đúng thứ phụ lục P1 của CHARTER và `CLAUDE.md` mục 4 cấm tuyệt đối, và lớp chặn máy vẫn sống — đo lại trong lượt worker 2026-09-21 22:3xZ, `guard.mjs` chặn cả một lệnh **đọc** `.github/workflows/ci.yml`. Đi vòng qua hook bằng công cụ khác là lách lớp chặn, không làm. Bài kiểm cần chủ dự án chạy, hoặc cần một ngoại lệ hẹp có thời hạn trong `guard.mjs` qua PR `owner-merge` — nới lớp chặn là CHARTER 2.3 nhóm 5. Issue `🤖 [QĐ]` của mục `VF-G10` nêu ba phương án; lời hứa "đề xuất riêng" về G10 có từ issue #5 (2026-09-20) và tới nay mới thực hiện.
- ⬜ **Dữ liệu gián tiếp, không phải kết luận:** tới `c7179c6`, đúng 2 commit trong lịch sử `main` ghi vào `.github/`, cả hai của `crux-sync` (PAT `WORKFLOW_SYNC_TOKEN`); không commit nào của phiên agent. Đó là hệ quả của chính lệnh cấm, nên độ tin cậy giữ nguyên `tài liệu nói vậy`.
