...
Lệnh này chạy lại **bài kiểm** của những giả định tự khai `**Kiểm tự động:**` ở mục của mình, in ra ba loại kết luận — `khớp`, `nâng` (có thêm bằng chứng, sửa sổ là đủ), `sai` (đổi trạng thái) — và với mỗi giả định `sai` thì **in sẵn thân issue `🤖 [QĐ]` kèm danh sách phần bị ảnh hưởng**, lấy nguyên từ cột *Phần phụ thuộc* của chính giả định đó. Giả định nào không tự kiểm được thì được liệt kê thành "cần người", không im lặng bỏ qua.
...
- **Độ tin cậy:** **`đã kiểm một phần`**
  ...
  **Bằng chứng, 2026-09-21 (routine, lần quan sát tự động đầu tiên):** `pnpm recheck:assumptions` quét các commit trên `origin/claude/*` chưa vào `main` và thấy **cả hai commit do routine tạo đều mang trailer**: `b244b1e` (routine `crux-integrator`) và `42b24b8` (routine `crux-worker-2`). Phần "chưa kiểm" của giả định này thu hẹp lại còn **thread**.
...
- **Kiểm tự động:** `trailer-commit-routine` — quét 14 ngày lịch sử git của mọi nhánh trên `origin`, và đòi **mọi commit do agent soạn message** (có `Co-Authored-By` nhắc tới Claude) đều mang `Claude-Session`. Commit do **công cụ** tạo — merge commit của `integrator-resolve.ts`, commit squash do GitHub tạo lúc merge PR — không mang và không thể mang trailer, vì message của chúng không đi qua agent; bài kiểm báo riêng chúng thành ghi chú thay vì tính vào kết luận. Hệ quả cần nhớ: **trên `main` gần như không có trailer nào**, vì repo merge bằng squash — muốn kiểm G14 thì phải đọc commit trên nhánh PR, không đọc `main`.
