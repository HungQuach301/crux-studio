...
- risk: low
- status: review
- hold: vế "tài khoản **có** Claude Code Projects" nằm ở trang cấu hình tài khoản, agent không thấy
...
- risk: low
- status: review
- hold: chuỗi `automerge` → `labels.yml` và `automerge` → `sync-workflows.yml` chưa lần nào quan sát được bằng chạy thật
...
- risk: high
- status: review
- hold: chưa đọc toàn văn NĐ 68/2026/NĐ-CP và NĐ 141/2026/NĐ-CP, chưa chạy thật đầu cuối — cần danh tính chủ dự án
...
- risk: high
- status: review
- hold: chưa đăng tin (đăng tin là cam kết trả tiền cho người thật, `irreversible` nhóm 1) nên chưa ai nhận việc
...
- risk: high
- status: review
- hold: ngữ cảnh **thread** chưa kiểm; hai luật `deny` cho công cụ MCP cố ý không thử
