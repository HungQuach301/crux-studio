...
- status: review
- **cửa merge:** chạy `node ops/invariants.protected-area.ts`, đừng đoán. PR `#120` chạm `CHARTER.md` mục ngoài 1 và 3 → `automerge-delayed`.
...
- tiêu chí xong:
  - ✅ **Đo trước, sửa sau** — `ops/scripts/delayed-gate-flow.ts` (`pnpm delayed:flow`), 15 test ở `ops/test/delayed-gate-flow.test.ts`. Chạy thật lúc `2026-09-22T06:46Z`: **14 PR** mang nhãn, **13 chắc chắn chưa từng** có khoảng đứng yên đủ 12 giờ; chỉ `#42` đạt (12,05 giờ). Bốn giới hạn khai thẳng trong docblock và trong KF-011: cận trên của đồng hồ · `headCommitsInWindow` đếm commit chứ không đếm lần push (cũng là cận trên) · giả định lịch sử nhánh không bị viết lại · PR **không đo được** ra `measured: false` và không bị cộng vào "chắc chắn chưa đủ". Yêu cầu gốc: một lệnh trong `ops/scripts/` trả lời được "cửa `automerge-delayed` có chảy không" bằng số — với mỗi PR mang nhãn đó: khoảng trống đầu-nhánh-không-đổi dài nhất, số lần đặt lại đồng hồ, và số giờ còn thiếu so với ngưỡng. Nguồn là `git log --first-parent` cộng nhãn, **không** đọc văn xuôi trong `note` của log.
  - ✅ **Bản tin nói số giờ thật** — `digest-metrics.ts` nhận kết quả đo đầu nhánh và in mục "Đang chờ merge": mỗi PR một dòng, gần tới hạn trước, kèm số giờ còn thiếu, khoảng đứng yên dài nhất so với ngưỡng, và "đồng hồ đặt lại N lần/24 giờ" từ lần thứ `RESET_ALERT_THRESHOLD` (3) trở đi. Không đo được thì in `CHƯA ĐO`, không in `0` (cùng luật với `P-007`); PR mang nhãn mà thiếu trong map đo hiện ra `CHƯA ĐO ĐƯỢC ĐẦU NHÁNH` thay vì âm thầm rơi khỏi con số. PR đang **xung đột** thay số giờ bằng lời nói về xung đột (đồng hồ không chạy, CHARTER 3.3), và dòng dặn cuối mục nói `Cửa delayed hiện CHƯA CHẢY` khi không PR nào đủ ngưỡng — không khuyên "không làm gì thì nó tự vào `main`" ngay dưới 13 dòng nói ngược lại. Phụ lục P2 sửa theo, dặn lấy số từ `pnpm delayed:flow` chứ đừng tự tính. Yêu cầu gốc: bản tin ngày (phụ lục P2, mục "Đang chờ merge") nói **số giờ còn lại thật**, tính theo đồng hồ đã bị đặt lại — chứ không phải giờ kể từ lúc gắn nhãn. Dòng nào đã bị đặt lại quá N lần thì nói ra. Hiện bản tin không phân biệt hai thứ đó, nên một PR kẹt vĩnh viễn trông giống một PR sắp tới hạn.
  - ⬜ **Bản sửa cơ chế — chờ issue `🤖 [QĐ]` #116.** Hai phương án, chọn bằng `[QĐ]`, đừng tự chọn:
  ...
  - ⬜ **Bằng chứng bằng chạy thật của bản sửa** — viết được sau khi #116 chọn phương án, vì bài kiểm khác nhau tuỳ (A) hay (B). Dựng lại đúng hình dạng — một PR `automerge-delayed` CI xanh, rồi một commit gộp bước 0 — và chạy `ops/invariants.merge-gate.ts`. Trước khi sửa: `outcome: "wait"`, `hoursLeft` quay về 12. Sau khi sửa: `merge`. Kèm **ca âm**: một commit có nội dung thật thì đồng hồ **phải** tính lại, nếu không (B) biến thành "merge bất chấp mọi thay đổi".
  - ⬜ **Việc nhỏ còn nợ, vòng soát `#120` nêu, không chặn tiêu chí nào:** dòng "Đang chờ merge" chưa có phần "chạm gì trong vùng bảo vệ", chưa có "· N lượt liên tiếp không tự giải được", chưa có các dòng `owner-merge` mà phụ lục P2 đòi trong cùng mục; 14 dòng delayed làm bản tin vượt mức ~25 dòng của P2 (nên cắt còn 5 dòng gần tới hạn nhất cộng "và N PR khác"); `probeOrigins` và `probeDelayedCommits` mỗi lượt `git fetch` hai lần cho tập ref chồng nhau. Ghi ra đây để không rơi mất.
  - ⬜ **Ngoài mục này, cần một mục riêng:** lệnh trong `CLAUDE.md` mục 1 và phụ lục P1 bước 7 thiếu `--base-charter`, nên mọi PR chạm `CHARTER.md` chạy đúng như tài liệu viết sẽ ra `owner-merge` (lớp chặn ngả về phía người khi không đọc được diff) thay vì `automerge-delayed`. Lỗi tài liệu có sẵn trên `main`, không do PR này gây ra, nhưng nó làm worker gắn sai nhãn một nhịp.
  - ✅ Ghi kết quả vào `ops/known-failures.md` **KF-011** — ba bullet mới: phép đo đã có kèm số, giới hạn của phép đo, và dòng **Máy chặn từ nay** chuyển từ "chưa có" sang "chưa đủ" (đo chứ chưa chặn, chặn chờ #116).
