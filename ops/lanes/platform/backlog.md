...
- tiêu chí xong:
  ...
  - ✅ `ops/scripts/telemetry-beat.ts` — vế ghi: nhánh `claude/telemetry`, thư mục `heartbeat/`, một file cho mỗi lượt (tên lấy nguyên từ `step0LogPath` của kernel, nên hai worker chồng nhau không chạm cùng file). Chỉ dòng bước 0 được vào: một dòng khác là **nhịp tim giả, mới hơn sự thật**, tức hướng làm người canh câm.
  ...
  - ✅ **Bộ lọc `ref` của dòng bước 0 chỉ còn MỘT bản.** Trước mục này nó nằm ở hai chỗ, hai ngôn ngữ: hằng `STEP0_REF_PATTERN` (TypeScript) và một biểu thức `jq` chép tay trong YAML — bản chép đã lệch thật một lần (thiếu `P1-step0-` và `P3-daily-`). Bản `jq` bị **bỏ hẳn**; `ops/test/lane-heartbeat.test.ts` đổi việc từ *"hai bản phải giống nhau"* sang *"chỉ được có một bản"*, tức chặt hơn.
