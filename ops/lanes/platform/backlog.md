...
**Chưa kiểm được ở đây, và đó là bản chất của mục này:** `smoke-workflows.yml` chỉ chạy lần đầu **sau khi** PR này merge và sync chép xong. Lần chạy đó là bằng chứng thật; mọi thứ ở đây là bằng chứng ở chỗ rẻ hơn (17 test cho phần quyết định, 4 bài phá thử đo được là đỏ đúng lúc). Lượt worker sau nên đọc đúng lần chạy đó trước khi coi mục này `done`.
