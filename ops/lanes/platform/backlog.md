...
- tiêu chí xong:
  - ✅ **Đo trước, sửa sau** — `ops/scripts/delayed-gate-flow.ts` (`pnpm delayed:flow`), 15 test ở `ops/test/delayed-gate-flow.test.ts`. Chạy thật lúc `2026-09-22T06:46Z`: **14 PR** mang nhãn, **13 chắc chắn chưa từng** có khoảng đứng yên đủ 12 giờ; chỉ `#42` đạt (12,05 giờ). Giới hạn khai thẳng trong docblock: khoảng đứng yên dài nhất là **cận trên** của đồng hồ, nên chiều "ngắn hơn ngưỡng ⇒ chắc chắn chưa mở được" là chắc, chiều ngược lại chỉ là "không loại trừ được". Yêu cầu gốc: một lệnh trong `ops/scripts/` trả lời được "cửa `automerge-delayed` có chảy không" bằng số — với mỗi PR mang nhãn đó: khoảng trống đầu-nhánh-không-đổi dài nhất, số lần đặt lại đồng hồ, và số giờ còn thiếu so với ngưỡng. Nguồn là `git log --first-parent` cộng nhãn, **không** đọc văn xuôi trong `note` của log.
  - ✅ **Bản tin nói số giờ thật** — `digest-metrics.ts` nhận kết quả đo đầu nhánh và in mục "Đang chờ merge": mỗi PR một dòng, gần tới hạn trước, kèm số giờ còn thiếu, khoảng đứng yên dài nhất so với ngưỡng, và "đồng hồ đặt lại N lần/24 giờ" từ lần thứ `RESET_ALERT_THRESHOLD` (3) trở đi. Không đo được thì in `CHƯA ĐO`, không in `0` (cùng luật với `P-007`). Phụ lục P2 sửa theo, dặn lấy số từ `pnpm delayed:flow` chứ đừng tự tính. Yêu cầu gốc: bản tin ngày (phụ lục P2, mục "Đang chờ merge") nói **số giờ còn lại thật**, tính theo đồng hồ đã bị đặt lại — chứ không phải giờ kể từ lúc gắn nhãn. Dòng nào đã bị đặt lại quá N lần thì nói ra. Hiện bản tin không phân biệt hai thứ đó, nên một PR kẹt vĩnh viễn trông giống một PR sắp tới hạn.
