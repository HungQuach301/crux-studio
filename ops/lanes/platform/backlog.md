...
- tiêu chí xong:
  ...
  - ✅ Bài **tái hiện lỗi** (**I2**) trong `ops/test/gpt-review.test.ts`: khẳng định prompt mang mức `CHẶN`/`NÊN SỬA`/`"không phát hiện"`/`CẤM tóm tắt` và KHÔNG còn `"đáng chú ý"`.
    ⚠️ Bài kiểm **đã được viết lại** ở lần gộp `P-054` rồi siết thêm ở `P-055`: nó neo chữ nguyên văn bằng `assert.equal(BLOCKING_LEVEL, 'CHẶN')` thay vì khớp `/\[CHẶN\]/`, cộng phép đòi các dòng ví dụ trong prompt phải qua được chính `parseReviewFindings`. Lời khai *"xanh 14/14"* ở đây là số của lúc mục này merge, không còn tái lập được trên `main` hiện tại.
...
- status: review
- nguồn: vòng soát ngữ cảnh sạch của lượt gộp `P-054`; `ops/known-failures.md` `KF-038`; PR `#258`/`#257`
- tiêu chí xong:
  ...
  - ⬜ **Máy chặn cho mã trùng — CHƯA làm, và đây là chỗ đáng làm nhất.** Kho hiện có **hai** `## KF-016` và **hai** `### P-028` nằm sẵn mà không cổng nào đỏ. Một cổng trong `pnpm check` bắt mã trùng (trong file, và giữa đầu nhánh với `main`) sẽ chặn cả ba ca — `P-028`, `P-051`, `KF-036`. Tách mục riêng: một mục = một PR.
- hold: chờ một mục riêng cho cổng chặn mã trùng, và chờ `🤖 [QĐ]` về cửa `open` + bước 6 (xem dưới). **Không** tự chuyển `done`.
