...
- tiêu chí xong:
  ...
  - ✅ Phá thử, mỗi phép đỏ đúng chỗ rồi khôi phục **28/28**: bỏ `owner-merge` khỏi `MERGE_GATE_LABELS` → **2** bài đỏ · `>=` thành `>` → 1 đỏ · bỏ `toLowerCase` → 1 đỏ · CLI không thoát 1 khi chỉ có `problems` → 1 đỏ · bỏ phép kiểm `isDraft` → 1 đỏ.
...
- tiêu chí xong:
  ...
  - ⬜ **Vế cơ chế `title ↔ tree` — phần CHÍNH của mục, và bốn ô trên KHÔNG che nó.** `claimCheck` phải phân biệt được *"không ai giữ mục này"* với *"không tiêu đề PR mở nào mang mã này, mà backlog thì có"*. Phán quyết thứ năm (tên gợi ý: `stale-id`) cho ca sau, và nó **không bao giờ** được đọc thành `free` — cùng luật 4 của `P-041` (*"đầu vào thiếu hay hỏng thì NÉM, không trả `free`"*). Nguồn để phân biệt có sẵn và không cần mạng: bên gọi đã có `readyNow`/`duplicateIds` của `backlog-status.ts`. Bài kiểm ghim đúng ca `P-028` `2026-09-26` bằng fixture.
  - ⬜ **`duplicateClaims` thấy va chạm ĐANG SỐNG, không chỉ kể lịch sử.** Đo được: trên ảnh chụp 232 PR nó trả 5 cặp, **cả 5 đều `firstMerged`**, và bỏ đúng cặp `#224 ↔ #274` đang mở. Hai PR cho cùng một mục mà mang hai mã khác nhau trong tiêu đề thì phải quy về **cùng một mục** trước khi đếm — quy bằng gì thì phải khai (ví dụ: tập file nội dung chồng nhau, hoặc mã mục mà diff của PR đổi tên trong backlog), và luật 3 của `P-041` (*"sóng nối tiếp KHÔNG phải va chạm"*) vẫn phải giữ, nên ca `P-014` (`#62`, `#196`, `#223`) phải là **ca âm**.
  ...
  - ⬜ **`ops/logs/platform/<mã>.jsonl` bị mục khác chiếm là vế THỨ BA, ngoài phạm vi mục này** — khai ra chứ không để im. Đó là lý do **thứ hai** `#274` phải đổi mã (`ops/logs/platform/P-028.jsonl` đã tồn tại, 2177 byte, thuộc mục `done` → đụng `D-C04`), và cổng dò tiêu đề Markdown ở trên không thấy nó. Đáng một mục riêng; trỏ sang đây khi tạo.
