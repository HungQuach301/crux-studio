...
- **Vì sao nó nặng hơn phần lớn lỗi khác:** `automerge.yml` merge bằng **squash**, nên thân commit của nhánh đi vào lịch sử `main`. Một PR revert **không** lấy lại được dòng đó — khác mọi thay đổi khác trong repo, vốn "nằm trong git nên revert được" (CHARTER 2.3). Commit đã vào rồi thì ở lại.
