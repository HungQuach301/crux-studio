...
- **Máy chặn từ nay — đổi hình dạng dữ liệu, mục `integration/I-020`:** "còn treo" nay là một **trường** `- hold: <lý do>` mà `ops/scripts/backlog-status.ts` đọc y như `- status:` và `- deps:` (`parseHoldField`). Có trường đó thì mục **không bao giờ** bị lật, bất kể thân mục viết gì — và nó chặn ở cả hai chỗ: `classify` và `applyFix`, tức cả chỗ thật sự ghi file. **Cả 35 mục nhóm `held` đã chuyển sang khai bằng trường** (đo được: `pnpm backlog:status` cho `heldByField` 35, `heldByProseOnly` **0**), và `ops/lanes/README.md` khai trường này cùng bảng với `status`/`deps`.
  Lưới lời văn **giữ lại** làm lớp thứ hai cho mục chưa kịp khai trường — gỡ nó là mở lại đúng lỗ vừa bịt — nhưng nó thôi là nguồn quyết định, và nó nay là **mẫu trên văn bản đã chuẩn hoá** (bỏ `` ` ``/`*`/`_`, gộp khoảng trắng, hạ hoa thường) thay vì danh sách chuỗi con. Nhờ đó ba biến thể mà bản ghi này khai là "còn lọt" nay **bị bắt**: "coi mục này **là** `done`" (chữ chèn vào giữa), "chỉ done khi" (`done` viết trần), "**chưa đóng**, dù PR đã merge".
  `pnpm backlog:status` in `heldByField` và `heldByProseOnly` — con số thứ hai là **nợ**: mục còn dựa vào lưới, và nó phải nhìn thấy được thì mới trả được.
- **Bài kiểm, trong `ops/test/backlog-status.test.ts` (`pnpm test`):**
  - `HOLD_MARKERS: ba biến thể lời văn lọt lưới lần hai — E-001, P-010, P-007` — ba câu thật là ba `assert`, **nguyên văn** chứ không rút gọn cho vừa chuỗi.
  - `HOLD_MARKERS: ba biến thể từng lọt lưới nay bị bắt — mục I-020 đã xong` — ba `assert` **đã đổi từ `false` sang `true`**. Đó là thước đo mà bản ghi này đặt ra, và nó nay đọc đúng chiều.
  - `parseHoldField`, `heldBy`, `classify`, `applyFix` và `reviewFindings` mỗi hàm một bài, kèm **test âm**: mục không khai trường mà thân mục mang câu treo vẫn không bị lật (lưới còn sống); mục sạch cả hai đường vẫn lật bình thường; `- hold:` trần không tính là khai.
- **Giới hạn còn lại, khai chứ không giấu:** lưới lời văn vẫn **không hội tụ** — câu thứ tư viết bằng chữ khác nữa vẫn lọt, và mẫu rộng hơn chuỗi con chỉ đẩy mốc đó ra xa, không xoá nó. Đó **không** phải chỗ để vá tiếp: gặp một ca lọt thì **khai trường `- hold:` cho mục đó**, đừng thêm mẫu. Luật một dòng của bản ghi này không đổi, và nay có máy đỡ: **một mục không muốn bị lật phải nói ra bằng một trường, không bằng một câu.**
