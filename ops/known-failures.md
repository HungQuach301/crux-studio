...
- **Máy chặn từ nay:** `ops/test/integrator-clean-merge-lockfile.test.ts`, nằm trong `pnpm test` nên `pnpm check` chặn trước khi PR tới GitHub. Bốn bài kiểm: ca hỏng tái hiện được (đỏ nếu ngày nào git hoặc pnpm đổi hành vi tới mức không tái hiện được nữa — lúc đó cơ chế nên được gỡ, không giữ vì quán tính); gộp sạch có chạm lockfile thì tạo lại, kiểm lại rồi mới commit; cổng vẫn đỏ sau khi tạo lại thì huỷ gộp và không commit gì; và gộp **không** chạm lockfile thì không gọi `pnpm` lần nào — cổng là một lần cài thật, không được trả giá đó ở mọi lần gộp. Đã kiểm bằng đột biến: bỏ `guardLockfileAfterMerge` thì hai bài giữa đỏ.
