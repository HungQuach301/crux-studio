...
- **Phép đo đã có, 2026-09-22 (lượt `crux-worker-1`, PR `#120`):** `ops/scripts/delayed-gate-flow.ts` (`pnpm delayed:flow`) trả lời bằng số, nguồn là `git log --first-parent origin/main..<đầu nhánh PR>` cộng nhãn — **không** đọc văn xuôi trong `note`. Với mỗi PR `automerge-delayed`: khoảng đầu-nhánh-không-đổi dài nhất, số lần đồng hồ bị đặt lại trong 24 giờ, và số giờ còn thiếu so với ngưỡng. **Kết quả chạy thật lúc `2026-09-22T06:46Z`: 14 PR mang nhãn, `13/14` CHẮC CHẮN chưa từng có một khoảng đứng yên đủ 12 giờ.** Chỉ `#42` đạt (12,05 giờ) — đúng PR mà bullet trên đã mổ, và nó vẫn trượt vì lịch `cron` của `automerge.yml`. Đụng dày nhất: `#39` và `#56`, **23 commit mới/24 giờ**. Bản tin ngày nay in mục **"Đang chờ merge"** từ chính phép đo này (`digest-metrics.ts`), nên số giờ trên bản tin là giờ theo đồng hồ đã bị đặt lại, không phải giờ kể từ lúc gắn nhãn.
- **Giới hạn của phép đo, khai thẳng — bốn điều, không ba:** (1) khoảng đứng yên dài nhất là **cận trên** của số giờ đồng hồ từng chạy liên tục (CI còn phải xanh xong sau commit), nên "ngắn hơn ngưỡng ⇒ cửa chắc chắn chưa mở được" là kết luận **chắc**, còn chiều ngược lại chỉ là "không loại trừ được"; (2) `headCommitsInWindow` đếm **commit**, không đếm lần push — ba commit push chung một lần chỉ đổi đầu nhánh một lần, nên nó cũng là **cận trên**, và con số `23` phải đọc là "23 commit mới/24 giờ", không phải "23 lần đặt lại"; (3) phép đo giả định **lịch sử nhánh không bị viết lại** — một lần `rebase`/`amend`/force-push đặt `%cI` về hiện tại và làm khoảng đứng yên co lại, khi đó "chắc chắn chưa đủ" có thể chắc mà sai; (4) PR **không đo được** (nhánh không có commit nào ngoài `main`) ra `measured: false` và **không** được cộng vào `neverReached` — gộp hai thứ đó là tự tạo một ca Z mới để chữa một ca Z cũ, và vòng soát ngữ cảnh sạch của `#120` bắt đúng chỗ ấy ở bản đầu. Con số `29 lần/24 giờ` của `#39` ở bullet trên đo lúc `05:47Z` bằng cách khác; phép đo mới ra `23`. Hai con số không mâu thuẫn — khác mốc đo, khác phạm vi đếm. Từ nay lấy con số của `pnpm delayed:flow`, vì nó có test và tái lập được.
- **Máy chặn từ nay:** **chưa đủ.** Phép đo đã có (bullet trên, 20 test ở `ops/test/delayed-gate-flow.test.ts`), nhưng nó **đo** chứ không **chặn**. Bản sửa cơ chế — phương án (A) hay (B) — đang chờ trả lời ở issue `🤖 [QĐ]` **#116**, và bài kiểm dựng lại đúng hình dạng (PR `automerge-delayed` CI xanh + một commit gộp bước 0 → `merge-gate.ts` phải trả `merge`, kèm **ca âm** là commit có nội dung thật thì đồng hồ **phải** tính lại) chỉ viết được sau khi biết chọn phương án nào. Tới lúc đó KF này vẫn mở.
