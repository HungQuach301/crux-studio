...
- **Bản tin đã nói đúng số giờ (PR `#120`, lượt `crux-worker-1` 2026-09-22):** mục "Đang chờ merge" của `digest-metrics.ts` lấy số từ `gate-flow.ts`, tức theo **đồng hồ đã bị đặt lại**. Ba chỗ dễ nói sai đã có test chặn: không đo được in `CHƯA ĐO` chứ không in `0`; PR đang xung đột **thay** số giờ bằng lời nói về xung đột (đồng hồ không chạy, CHARTER 3.3); và khi không PR nào từng đạt ngưỡng thì dòng dặn nói `⚠️ Cửa delayed hiện CHƯA CHẢY`, **không** khuyên "không làm gì thì nó tự vào `main`". Chỗ cuối là chỗ câu chữ sai gây thiệt hại trực tiếp nhất — chủ dự án đọc bản tin đúng để quyết định *không làm gì*.
